            ...
            <div className="rounded-3xl shadow-2xl grid grid-cols-2 md:grid-cols-4 mt-6 p-2 bg-white">
                {
                    categories.map((cat, i) => (
                        <Link to={`/store/${cat.replace(" ", "-")}`} key={i} className={`px-10 h-40 text-center border-gray-200 ${i === 0 ? "border-r-2" : ""} ${i === 1 ? "md:border-r-2" : ""} ${i === 2 ? "border-r-2 border-t-2 md:border-t-0" : ""} ${i === 3 ? "border-t-2 md:border-t-0" : ""}`}>
